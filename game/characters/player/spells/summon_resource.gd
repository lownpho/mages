...
## lifetime runs out, splitting enemy aggro off the player. Each minion is a faction-
## flipped Enemy (see summon_minion) and its whole attack rides minion_weapon, reusing
## the weapon + bullet systems. The SpellResource's own base_damage / skill_scaling are
## unused — the minion's damage lives on minion_weapon's bullet. What makes it Halp vs
## Bzzz is the minion scene (art + FSM) and these numbers, not code.
