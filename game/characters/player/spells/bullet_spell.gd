...
##
## Three aim modes, all data (BulletSpellResource.aim_mode): it tracks the caster shot to
## shot, commits to the lane it started with, or paints from its own random bearing.
##
## Exclusive spells cancel it: SpellCaster calls interrupt() when a newer burst,
## cast or channel starts, which ends this one onto its full cooldown. Instant
## spells leave it firing. Faction-agnostic through CastContext.
...
func _ready() -> void:
	...
	_locked = data.aim_mode != BulletSpellResource.AIM_TRACK
	if data.aim_mode == BulletSpellResource.AIM_INDEPENDENT:
...
func _physics_process(delta: float) -> void:
	...
	if _cadence < data.shot_interval:
		return
	if caster.has_method("can_burst_fire") and not caster.can_burst_fire():
		return
	_fire()
	_cadence = 0.0
...
func _fire() -> void:
	...
	# committed at the start (aim_mode Lock or Independent), in which case every shot reuses it.
	...
	for i in dirs.size():
		...
		ctx.spawn_bullet(data.bullet, dirs[i], spawn_pos)
