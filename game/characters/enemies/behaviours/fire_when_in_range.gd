...
## only when the spell is off cooldown does it play `attack_anim` as a wind-up, firing when
...
func enter() -> void:
	...
	if animation_driven:
		# Rest in idle; physics_update kicks off a wind-up once the spell is ready. The
...
func physics_update(_delta: float) -> void:
	var player := target_or_go(out_of_range_state)
	if not player:
		return
	...
	if animation_driven:
		# Kick off a telegraphed shot when idle and the spell has come off cooldown. Once a
		# wind-up is running we're committed — the frame handlers finish it.
		if not _winding_up and _caster.ready_for(spell):
			_winding_up = true
			_fired = false
			creature.play(attack_anim)
	else:
...
func _fire(player: Node2D) -> void:
	_caster.cast(spell, aim_at(player))
...
# resting pose. The spell's cooldown gates the next wind-up in physics_update.
