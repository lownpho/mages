extends Node
## Headless weapon-spell burst test, driven through the real player scene and
...
## a cast-time spell, a channel, or a second weapon — CANCELS the live burst
## onto its full cooldown; instant spells stack on a firing burst; and
## burst_window caps an overlong burst. Slots live on two pages of three
## (cycle_spell_page flips which three the cast actions drive). Run:
##   godot --headless --path game res://tests/test_weapon_spell.tscn
...
var pew1: WeaponSpellResource
var pew2: WeaponSpellResource
...
func _ready() -> void:
	...
	await _test_weapon_cancels_weapon()
	await _test_instant_stacks()
	await _test_channel_cancels_burst()
	await _test_window_caps_burst()
...
func _count(spell: WeaponSpellResource) -> int:
...
func _test_full_burst() -> void:
	...
	caster._try_cast(0)  # re-cast while live must be a no-op
...
func _test_cast_cancels_burst() -> void:
	...
	caster._try_cast(2)  # fireball: cast_time > 0, must cancel the burst
...
func _test_weapon_cancels_weapon() -> void:
	spawned.clear()
	caster._try_cast(0)
	await _wait(0.3)  # ~2 pew1 shots
	caster._try_cast(1)  # pew2: a second weapon cancels the first
	var p1 := _count(pew1)
	if not _cooling(pew1) or _burst_live(pew1):
		fails.append("second weapon did not cancel the first onto cooldown")
	await _wait(pew2.shot_interval * pew2.max_shots + 0.3)
	if _count(pew1) != p1:
		fails.append("cancelled weapon kept firing")
...
func _test_instant_stacks() -> void:
	spawned.clear()
	caster._try_cast(0)
	await _wait(0.3)
	...
	if _cooling(pew1):
		fails.append("instant spell put the burst on cooldown")
	await _wait(pew1.shot_interval * pew1.max_shots + 0.3)
	if _count(pew1) != pew1.max_shots:
		fails.append("burst under an instant spell reached %d bullets, want %d" % [_count(pew1), pew1.max_shots])
...
func _test_channel_cancels_burst() -> void:
	...
	await _wait(0.3)
	GlobalInventory.cycle_spell_page()
	caster._try_cast(1)  # nope (page 1, slot 1): channeled, cancels at press
	var at_cancel := _count(pew1)
	if not _cooling(pew1) or _burst_live(pew1):
...
func _test_window_caps_burst() -> void:
	# A synthetic hose whose shots (100) can never finish inside its window:
	# the window must end the burst.
	var hose := WeaponSpellResource.new()
	hose.effect_scene = load("res://characters/player/spells/weapon_spell.tscn")
	hose.fire_pattern = SinglePattern.new()
	hose.bullet = BulletResource.new()
	hose.bullet.speed_tiles = 14
	hose.bullet.range_tiles = 8
	hose.shot_interval = 0.2
	hose.max_shots = 100
	hose.burst_window = 0.55
	hose.cooldown = 1.0
	GlobalInventory.spell_slots.at(5).set_item(hose)
	GlobalInventory.cycle_spell_page()
	spawned.clear()
	caster._try_cast(2)  # hose (page 1, slot 2)
	await _wait(hose.burst_window + 0.3)
	var shots := _count(hose)
	if shots < 2 or shots > 4:  # ~3 shots fit a 0.55s window at 0.2s cadence
		fails.append("window-capped burst fired %d shots, want ~3" % shots)
	if not _cooling(hose):
		fails.append("window expiry did not put the burst on cooldown")
	if _burst_live(hose):
		fails.append("burst effect survived its window")
