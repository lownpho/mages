extends Node
## Headless check on the HUD's pad focus ladder: the strip's spell slots, bag slots and
## buttons must chain as ONE 3-wide grid so dpad navigation is deterministic instead of
...
func _ready() -> void:
	...
	var nav := []
	for group in [ui.get_node("%EquipSpells"), ui.get_node("%Bag")]:
		nav.append_array(group.get_children())
	for name_ in ["%BestiaryButton", "%MapButton", "%QuitButton"]:
		nav.append(ui.get_node(name_))

	var expected := GlobalInventory.SPELL_SLOT_SIZE + GlobalInventory.BAG_SIZE + 3
