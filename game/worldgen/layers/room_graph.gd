...
##   2. Spanning tree: canonical edge list (units row-major, then neighbour), Fisher-Yates shuffle,
...
##      placed type; pass 3 weighted fill, one sample-roll per remaining unit.
extends RefCounted
...
static func build(world_spec: WorldSpec, biome_coord: Vector2i, config: GenConfig) -> BiomeGraph:
	...
	var t := config.ROOM_SLOT_SIZE
	var bid := world_spec.biome_at(biome_coord)
	var biome := config.biome_by_id(bid)
	var parts: Array[int] = [world_spec.world_seed, WgHash.NS_ROOM_GRAPH, biome_coord.x, biome_coord.y]
	...
	owner.resize(s * s)
	owner.fill(-1)
	var unit_top: Array[Vector2i] = []   # local slot top-left per unit
	var unit_size: Array[Vector2i] = []
	for ly in s:
		for lx in s:
			var idx := ly * s + lx
			if owner[idx] != -1:
			...
			if unique_here.has(here):
				_claim(owner, unit_top, unit_size, s, here, Vector2i.ONE)
				continue
			# Eligible slot: consume exactly one merge roll regardless of outcome (spec §7.1).
			var shape := Vector2i.ONE
			if WgHash.chance(rng, config.threshold_merge):
				for sh in [Vector2i(2, 2), Vector2i(2, 1), Vector2i(1, 2)]:
					if _shape_fits(owner, unique_here, s, here, sh):
						shape = sh
						break
			_claim(owner, unit_top, unit_size, s, here, shape)

	var n_units := unit_top.size()

	# --- Step 2: spanning tree ------------------------------------------------------------------
	# Canonical edge list: unit indices are already row-major by top-left, so (a<b) sorted by (a,b)
	# is "by top-left unit, then neighbour" (spec §7.2).
	var edges := _enumerate_edges(owner, s, n_units)   # Array of Vector2i(a, b), a < b
	...
	parent.resize(n_units)
	for i in n_units:
		parent[i] = i
	var tree: Dictionary = {}   # edge key -> true; lookup-only
	for e in order:
		if _uf_union(parent, e.x, e.y):
			tree[_ekey(e, n_units)] = true

	# --- Step 3: loops + passage geometry -------------------------------------------------------
	var kept: Array = []   # of {edge: Vector2i, from_tree: bool}, in canonical order
	for e in edges:
		var is_tree: bool = tree.has(_ekey(e, n_units))
		if is_tree:
			kept.append({"edge": e, "from_tree": true})
		elif WgHash.chance(rng, config.threshold_loop):
			kept.append({"edge": e, "from_tree": false})

	var unit_passages: Array = []   # per unit: Array[RoomSpec.Passage]
	for i in n_units:
		unit_passages.append([])
	for entry in kept:
		var e: Vector2i = entry["edge"]
		_carve_passage(unit_top, unit_size, t, config.DOOR_WIDTH, biome.openness_threshold, rng,
				e.x, e.y, entry["from_tree"], unit_passages)

	# Border-contract crossings (spec §6/§7.3) — forced external doors; NO biome RNG consumed here.
	_append_contracts(world_spec, biome_coord, s, t, owner, unit_top, unit_passages)

	# --- Step 4: type assignment ----------------------------------------------------------------
	var types: Array[StringName] = []
	types.resize(n_units)
	types.fill(&"")
	# Pass 1: world-unique stamps (the host slot is always its own 1x1 unit).
	for ur in world_spec.unique_rooms:
		if ur.biome_coord == biome_coord:
			types[owner[ur.local_slot.y * s + ur.local_slot.x]] = ur.type_id
	# Pass 2: biome-unique types, sorted by type_id, each onto a uniformly-chosen free unit.
	var biome_unique: Array[StringName] = []
	for rt in config.room_types:
		if rt.unique_scope == RoomTypeDef.UniqueScope.BIOME and bid in rt.allowed_biomes:
	...
	# Pass 3: weighted fill from the biome's room-type table, in canonical unit order.
	var table: Array = []       # applicable entries (type allowed in this biome)
	for e in biome.room_type_table:
		var rt := config.room_type_by_id(e.type_id)
		if rt != null and bid in rt.allowed_biomes:
			table.append(e)
	...
	for i in n_units:
		...
		var chosen: StringName = &"traversal"
		for e in table:
			if placed.get(e.type_id, 0) >= e.max_per_biome:
				continue
			r -= e.weight
	...
	graph.size_slots = s
	graph.slot_to_unit = owner
	for i in n_units:
		var spec := RoomSpec.new(biome_coord * s + unit_top[i], unit_size[i], bid)
		spec.type_id = types[i]
		spec.passages = unit_passages[i]
		graph.units.append(spec)
	return graph


# --- Merge helpers ------------------------------------------------------------------------------

static func _claim(owner: PackedInt32Array, unit_top: Array[Vector2i], unit_size: Array[Vector2i],
...
## Record one internal passage on both units of edge (ia, ib) with geometrically consistent offsets.
static func _carve_passage(unit_top: Array[Vector2i], unit_size: Array[Vector2i], t: int,
	...
	var seg_hi: int
	var edge_a: int       # this unit's wall-segment start, in slots (along the border axis)
	var edge_b: int
	...
	var base_a := (seg_lo - edge_a) * t     # shared-segment start, in this unit's edge frame (tiles)
...
## Append border-contract crossings (spec §6) as forced external DOORs on the correct edge units.
## A crossing's slot_index runs along the shared border; its tile_offset is relative to that SLOT's
## wall start. We map it to the unit owning that border slot and rebase the offset onto the UNIT's
## wall-segment start (crucial for merged units, whose start is not the slot's).
static func _append_contracts(world_spec: WorldSpec, biome_coord: Vector2i,
		s: int, t: int, owner: PackedInt32Array, unit_top: Array[Vector2i], out: Array) -> void:
	var neighbours := [
	...
	for nb in neighbours:
		...
		for c in world_spec.get_contract(biome_coord, other):
			...
			var edge_start: int   # unit wall-segment start along the border axis (slots)
			var unit_i: int
			if side == WorldSpec.SIDE_EAST:
				local_slot = Vector2i(s - 1, c.slot_index)
				unit_i = owner[local_slot.y * s + local_slot.x]
				edge_start = unit_top[unit_i].y
				...
				unit_i = owner[local_slot.y * s + local_slot.x]
				edge_start = unit_top[unit_i].x
			else:
				local_slot = Vector2i(c.slot_index, 0)
				unit_i = owner[local_slot.y * s + local_slot.x]
				edge_start = unit_top[unit_i].x
			var offset: int = (c.slot_index - edge_start) * t + c.tile_offset
			out[unit_i].append(RoomSpec.Passage.new(side, RoomSpec.KIND_DOOR, offset, c.width, true, false))
