...
## Output of Layer 3+4 for one room unit (spec §10.5), LRU-cached by the streamer. The spec
## fields are unit_id / attempt_used / tile_grid / reachability_map / spawns; width, height,
## protected_map and the id fields are implementation extras (debug views + generator input).
extends RefCounted

var unit_id: Vector2i
...
var spawns: Array = []                        ## data only; populated by Layer 4 (Task 7)
