...
## `centred` pins the group's first entity to the room centre (the boss); with `pack_spread` set
## that centre is also what the rest of the pack clusters around.
extends Resource
...
@export var centred := false             ## place the group's FIRST entity on the room centre (boss); the rest follow the usual draws
