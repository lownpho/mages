class_name BiomeDef
## What a biome IS at the config level (spec §10.4). One per world cell. Spawn/loot tables
...
@export_range(0.0, 1.0, 0.01) var open_passage_chance: float = 0.5   ## P(passage is OPEN vs DOOR) in L2 (spec §7.3)
@export_range(0.0, 1.0, 0.005) var decor_density: float = 0.0  ## per-tile P(FLOOR -> DECOR_FLOOR) (spec §8.1.5)
@export var room_type_table: Array[RoomTypeTableEntry] = []    ## weighted fill table — the SOLE opt-in for non-unique room types (spec §7.4)
@export var spawn_tables: Array[RoomSpawnTable] = []           ## one enemy table per room type
@export var loot_tables: Array[RoomLootTable] = []             ## one loot table per room type

## Logical-class → tileset art mapping. PRESENTATION only: deliberately outside CONFIG_HASH
## (spec §13 — tile-art selection is not part of the deterministic world), so it is NOT folded
...
## display_color and presentation are deliberately absent — both are presentation (spec §13).
