...
## (ZOOM_TILES_PER_PX), and the wall overlay only renders at the closest zoom. Live enemies
## show only inside discovered rooms, so nothing leaks through fog of war.

const ZOOM_TILES_PER_PX: Array[int] = [1, 2, 4, 8, 16, 32]
const WALLS_MAX_TPP := 1    ## wall overlay visible only at zooms this fine or finer
...
func _draw() -> void:
	...
	_draw_layer(_state.floor_texture, region, tpp)
	if tpp <= WALLS_MAX_TPP:
		_draw_layer(_state.wall_texture, region, tpp)
...
## clamped to the world so out-of-bounds sampling never smears edge pixels.
func _draw_layer(tex: ImageTexture, region: Rect2, tpp: int) -> void:
	var vis := region.intersection(Rect2(Vector2.ZERO, Vector2(_state.world_tiles)))
	if not vis.has_area():
		return
	draw_texture_rect_region(tex, Rect2((vis.position - region.position) / tpp, vis.size / tpp), vis)
