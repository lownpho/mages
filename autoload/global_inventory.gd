...
func swap_items(from_slot: SlotPosition, to_slot: SlotPosition) -> bool:
	...
	GlobalEvent.inventory_updated.emit()
...
func print_inventory() -> void:
	...
	# Print array-based slots (Bag and Spells)
	for type in slots.keys():
		if is_array_slot(type):
			print("\n%s:" % GlobalDefs.ItemType.keys()[type])
			for i in range(slots[type].size()):
				var item = slots[type][i]
				print("[%d] %s" % [i, item.scene.resource_path.get_file() if item else "---"])
